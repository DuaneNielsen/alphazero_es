...
import numpy as np
import eval.opening_book as opening_book
    ...
    def single_rollout(self, rng_input, num_simulations, policy_params):
        ...
        def policy_step(state_input, _):
            ...
            if self.stochastic_actions:
                action = jax.random.categorical(rng_action, jnp.log(policy_output.action_weights), axis=-1).squeeze(0)
            else:
...
for gen in range(args.num_generations):
    ...
    reshaped_params = jax.tree.map(lambda x: x.reshape((args.popsize//2, 2) + x.shape[1:]), reshaped_params)
    ...
    fitness = cum_ret.mean(axis=1)
    fitness= fitness.reshape(args.popsize)
    ...
    if (gen + 1) % print_every_k_gens == 0:
        ...
        eval_score, state, longest_run_index, longest_run_steps = \
            evaluate(rng_rollout, 100, args.num_simulations)
